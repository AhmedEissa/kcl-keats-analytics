...
function showUser()
{
   ...
   if(getUserId() != $USER->id)
      echo '<p>User overridden: ' . get_username_from_userid(getUserId()) . '</p>';
}
...
function get_course_modules_recently_accessed($courseid, $number = DISPLAY_LIMIT_MAIN, $userid = NULL)
{
   ...
   foreach($LOG['Index'] as $index)
   {
      if( ! isset($LOG['UserID'][$index]))
         $LOG['UserID'][$index] = get_userid_from_username($LOG['Users'][$index]);

      // skip if outside date limits
      if($FILTERDATES && $from && $to)
      {
         if($LOG['Unix_Date/Time'][$index] < $from || $LOG['Unix_Date/Time'][$index] > $to)
            continue;
      }
      ...
      NULL && in_array($LOG['Activity'][$index], $module_types)
...
function get_course_module_accesses($courseid, $moduleid, $uniques = true, $role = "Student"/*||"All"*/)
{
   ...
   foreach($LOG['Index'] as $index)
   {
      ...
      if($uniques)
         if(in_array($LOG['UserID'][$index], $users))
            continue;

         // note user and count
      if( ! in_array($LOG['UserID'][$index], $users))
         $users[] = $LOG['UserID'][$index];
      $accesses++;
   }
   return $accesses;
}
...
/** show module accesses breakdown table v1
 *
 * @param courseid
 * @return void -> html
 */
function show_module_accesses_breakdown_v1($courseid)
{
    global $OUTPUT, $MODE;
 ...
 * TODO Fix/use $LOG rather (needs other users included?)
 */
function user_has_accessed_course_sp($courseid, $userid)
{
   global $DB;
   global $FILTERDATES, $from, $to;
...
function get_course_assignment_submission_data($courseid, $userid = NULL)
{
   ...
   for($c = 0; $c < count($assignments); $c++)
   {
      ...
      foreach($results as $result)
      {
         if( ! is_student_on_course($courseid, $result->userid))
            continue;
      ...
      else
      {
         if($assignment['done'] == "submitted")
         {
            ...
            else
            {
               ...
               if($assignment['percentage_submitted'] > 0)
                  $title .= ' but not you';
               $indicatorclass = "notdone";
            }
      }
...
function display_progress_tracker_chart_include($courseid)
{
   ...
   echo '<h3>';
   if($FILTERDATES && $from && $to)
      echo 'First accessed activities';else
      echo 'Recently accessed activities';
   echo '</h3>' . "\n";
   ...
   foreach($recent_modules as $moduleid=>$recent_details)
   {
      ...
      if($MODE == "Staff")
         echo '<td class="student">' . $recent_details['student'] . '</td>';
      ...
      echo '<td class="percentage" '; echo ' title = "Accessed: ' . $recent_details['datetime'] . '"'; echo'>' . $percentage . '</td>';
      $accessed = user_has_accessed_module($courseid, $moduleid, getUserId());
      $data[$c]['accessed'] = $accessed;
      if($MODE == "Student")
         echo '<td class="accessed">' . ($accessed? "Yes": "No") . '</td>';
      echo '</tr>';
      $c++;
   }
   echo '</tbody>';
   ...
   if($MODE == "Staff")
      echo '<br clear="all"/><br/><p><small>You can also view the course <a href="' . $CFG->wwwroot . '/report/outline' . '?id=' . $courseid . '">Activity report</a> and <a href="' . $CFG->wwwroot . '/report/log' . '?id=' . $courseid . '">Log</a>.</small></p>';
   ...
   if($MODE == "Staff")
      echo '<br clear="all"/><br/><p><small>Note: Requires <a href="http://docs.moodle.org/23/en/Activity_completion">"Activity completion" enabled and setup</a> for activities as required.</small></p>';
   ...
   foreach($assignments as $assignment)
   {
      ...
      if($MODE == "Student")
      {
         if($assignments[$c]['done'] == "submitted")
            $data[$c]['student_percent'] = 100;
         else
            if($assignments[$c]['done'] == "draft")
               $data[$c]['student_percent'] = 50;
            else
               $data[$c]['student_percent'] = 0;
      }
...
function show_course_views_summary($courseid, $LDate)
{
   ...
   // get list of enrolled userids   user_has_accessed_course_sp
   $users_on_course = get_course_users($courseid);
   ...
   $htmlTable = $htmlTable . "	<tr>
		            <th class='thTitle'>As on $LDate</th>
